...
// Toggle label now reflects current mode
...
$('#compareBtn').addEventListener('click',()=>{
  if(!lastAttempt || !prevAttempt){
    alert('Record at least two attempts to compare.');
    return;
  }
...
// Recording (unchanged)
let recStream, mediaRecorder, recChunks=[];
async function startRecording(){
  try{
    recStream = await navigator.mediaDevices.getUserMedia({ video:true, audio:true });
  }catch(err){ alert('Camera/mic permission needed.'); throw err; }
